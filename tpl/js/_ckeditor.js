...
(function (window) {
  ...
  function attach(editor) {
    ...
    // CKEditor 의 좌/중/우 Justify 명령을 가로채 oembed wrapper 에 직접
    // data-oembed-align 을 토글한다. wrapper 는 widget 으로 등록돼 있지
    // 않아 기본 동작은 부모 단락의 text-align 만 바꾸므로, SDK 임베드가
    // 자체 margin:auto 를 갖는 케이스에선 시각 차이가 거의 없다.
    // 별도 toolbar UI 추가 없이 기존 좌/중/우 버튼만으로 임베드 정렬을
    // 제어하기 위함. justifyblock(양쪽정렬) 은 임베드에 의미가 없어
    // 가로채지 않고 기본 동작 통과시킨다.
    var oembedAscendant = function (n) {
      return n.type === window.CKEDITOR.NODE_ELEMENT
        && n.hasAttribute && n.hasAttribute('editor_component')
        && n.getAttribute('editor_component') === 'oembed';
    };

    editor.on('beforeCommandExec', function (evt) {
      var name = evt.data.name;
      if (name !== 'justifyleft' && name !== 'justifycenter' && name !== 'justifyright') {
        return;
      }
      var sel = editor.getSelection();
      var startEl = sel && sel.getStartElement();
      var wrapper = startEl && startEl.getAscendant(oembedAscendant, true);
      // contenteditable=false 위젯 단일 선택 시 startElement 가 부모 단락을
      // 가리키는 케이스가 있어 getSelectedElement 도 fallback 으로 검사.
      if (!wrapper && sel && sel.getSelectedElement) {
        var picked = sel.getSelectedElement();
        if (picked && picked.hasAttribute && picked.hasAttribute('editor_component')
            && picked.getAttribute('editor_component') === 'oembed') {
          wrapper = picked;
        }
      }
      if (!wrapper) {
        return; // wrapper 미발견 시 기본 Justify 동작 그대로 (= 일반 단락 정렬).
      }
      editor.fire('saveSnapshot');
      if (name === 'justifyleft') {
        wrapper.removeAttribute('data-oembed-align');
      } else if (name === 'justifycenter') {
        wrapper.setAttribute('data-oembed-align', 'center');
      } else {
        wrapper.setAttribute('data-oembed-align', 'right');
      }
      editor.fire('saveSnapshot');
      evt.cancel();
    });

    // selection 이 wrapper 위/안에 있을 때 toolbar 의 좌/중/우 버튼이
    // 현재 정렬 상태를 반영하도록 동기화. wrapper 가 아닌 일반 단락에서는
    // 이 핸들러가 손대지 않으므로 CKEditor 의 기본 상태 동기화가 그대로 유효.
    editor.on('selectionChange', function () {
      ...
      var TS = window.CKEDITOR.TRISTATE_ON, TF = window.CKEDITOR.TRISTATE_OFF;
      var align = wrapper.getAttribute('data-oembed-align');
      var states = {
        justifyleft: !align ? TS : TF,
        justifycenter: align === 'center' ? TS : TF,
        justifyright: align === 'right' ? TS : TF
      };
      for (var key in states) {
        if (Object.prototype.hasOwnProperty.call(states, key)) {
          var cmd = editor.getCommand(key);
          if (cmd) {
            cmd.setState(states[key]);
          }
        }
      }
    });
