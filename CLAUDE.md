...
2. **Conversion (server)** — `controllers/Controller.php::procOembedFetch`
   ...
   - The wrapper carries: `editor_component="oembed"` (CKEditor widget hint), `contenteditable="false"` (single-block selection inside the editor), `data-url` (debugging/tooling reference), `data-oembed-type` (one of `social` / `multimedia` / `card` — used by editor.css for type-specific visualization), and on provider-matched embeds also `data-oembed-provider` (class basename like `Instagram`/`Facebook`/`X`/`Youtube` — used by editor.css to pick the right label/badge). 임베드 차원/카드 메타 등은 모두 inner HTML 에 들어 있으므로 wrapper 에 중복 보관하지 않는다.
