...
저장된 마크업은 출력 시점에 `oembed.class.php` 의 `transHTML` 이 자동으로 다시
처리하므로, 통합 측에서 따로 출력 변환을 신경 쓸 필요는 없습니다.
...
저장 시 본문에는 `<div editor_component="oembed" data-...>` 가 그대로 남습니다.
글이 출력될 때 Rhymix 의 에디터 코어가 `oembed.class.php::transHTML($xml_obj)`
를 자동 호출해 다시 임베드 HTML 을 생성합니다.

따라서 통합 코드 측에서는:

- 저장 전 본문에서 oembed 마크업을 *지우거나 변형하지 마세요*.
- 출력 변환 로직을 따로 구현하지 마세요(자동 처리됩니다).
